...
default.punch=function(target,puncher,damage)
	...
	target:punch(puncher,1,{full_punch_interval=1,damage_groups={fleshy=damage or 1}},{x=0,y=0,z=0})
end
...
default.extract_meta=function(pos1,pos2,filename)
	if not filename then
		print("default.extract_meta: filename is missing")
		return false
	end
	...
	for z=0,pos2.z-pos1.z do
		local m1 = minetest.get_meta(apos(pos1,x,y,z)):to_table()
		...
		if inv or fields then
			m[x..","..y..","..z] = m1
		end
	end
	end
	end
	local fp = minetest.get_worldpath() .. "/schematics/"..filename..".mtsm"
	local f = io.open(fp, "w")
	f:write(minetest.serialize(m))
	f:close()


	minetest.log("default.extract_meta created:: "..fp)
	return true
end

default.insert_meta=function(pos,filepath)
	if not filepath then
		print("default.insert_meta: filepath is missing")
		return false
	end
	...
	for i,v in pairs(minetest.deserialize(f or "")) do
		local p = i.split(i,",")
